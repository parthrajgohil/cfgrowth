...
| Sender for agent leads (2026-09-25) | `parthraj.gohil@corefragment.com` connected to Saleshandy (id `Y8aL7kk3PN`, O365, health 86, sender name "Parthraj Gohil", no signature, 15/day, not in any sequence). Entra admin consent for the Saleshandy app was needed. Warm-up to run ~2 weeks before the test sequence starts (target start ≈ 2026-10-09). Note: this is the main domain; keep volume ≤20/day. **corefragment.com DMARC fixed** (verified on public resolvers). |
...
- Confirm Saleshandy warm-up is ON for parthraj.gohil@ (not visible via the API)
