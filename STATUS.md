...
| LinkedIn (2026-09-25) | (1) **LinkedIn touch** for no-email leads: HubSpot field `cf_linkedin_touch` (gate: agents set only `to_send`); hourly run drafts connection note ≤280 chars, InMail, and after-accept message and sends them to Teams; first one done for Legato. Pattern-guessing emails rejected by the CEO. (2) **Content engine**: company-page post Tue/Wed/Thu 11:47 IST (Tue insight + visual, Wed carousel, Thu news/people; video script on first Thursdays) → Teams. LaunchAgent `com.corefragment.linkedin-content` written; the CEO installs it. Images/videos are briefs (Teams messages are text-only); automatic image/PDF generation is a possible v2. |
